...
def haversine_vec(lat1, lon1, lat2_array, lon2_array, R=6371+506.7):
...
def compute_lat_lon_distances(alt_lat_idx, alt_lon_idx, doris_lat_inpo, doris_lon_inpo):
    ...
    alt_lon_rad = np.radians(alt_lon_idx)
    doris_lat_rad = np.radians(doris_lat_inpo)
    doris_lon_rad = np.radians(doris_lon_inpo)

    # 纬度差（单位：弧度）
    delta_phi = doris_lat_rad - alt_lat_rad
    # 经度差（单位：弧度）
    delta_lambda = doris_lon_rad - alt_lon_rad
    delta_lambda = (delta_lambda + np.pi) % (2 * np.pi) - np.pi
    # 沿纬线方向的距离
    dist_lat = R * delta_phi

    # 沿经线方向的距离，需要乘以 cos(平均纬度)
    mean_lat = (doris_lat_rad + alt_lat_rad) / 2
    dist_lon = R * delta_lambda * np.cos(mean_lat)

    return np.abs(dist_lat)/1000, np.abs(dist_lon)/1000
