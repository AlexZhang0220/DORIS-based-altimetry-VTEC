...
def plot_bias_std_vs_vtec(vtec, std, ref_vtec, label=None, color=None):
    bias = vtec - ref_vtec

    # 分 bin，例如每 2 TECU 分一段
    bins = np.arange(0, np.max(vtec) + 2, 2)
    ...
    for i in range(len(bins) - 1):
        mask = (vtec >= bins[i]) & (vtec < bins[i+1])
        if np.any(mask):
            mean_bias.append(np.mean(bias[mask]))
            mean_std.append(np.mean(std[mask]))
        else:
            mean_bias.append(np.nan)
            mean_std.append(np.nan)

    # 画线
    ax_bias.plot(bin_centers, mean_bias, label=label, color=color, linestyle='-')
    ax_std.plot(bin_centers, mean_std, label=label, color=color, linestyle='--')
...
def inpo_results_plot(start_year, start_month, start_day, proc_days, settings: list[str]):
    # settings: [min_obs, elevation, lat_range (sec_gap)]
    ...
    for i in range(proc_days):
        process_date = start_epoch + timedelta(days=i)
        year, month, day = process_date.year, process_date.month, process_date.day
        doy = process_date.timetuple().tm_yday
        doy_list.append(doy)
        epoch_str = f'y{year}_d{doy}'
        min_obs_str, ele_str, division_str = settings
        doris_name = f'./InpoResults/DORISNew/{epoch_str}_{division_str}_{ele_str}_{min_obs_str}.npy'
        doris_rms.append(np.load(doris_name)[0][0][0][0])
        percent.append(np.load(doris_name)[0][0][0][1])
        gim_name = f'./InpoResults/GIM/{epoch_str}_{division_str}_{ele_str}_{min_obs_str}.npy'
        gim_rms.append(np.load(gim_name)[0][0][0])

    doris_rms = np.array(doris_rms)
    gim_rms = np.array(gim_rms)
    percent = np.array(percent)
    percent_min = np.min(percent * 100)
    percent_max = np.max(percent * 100)
    mean_diff = np.mean(doris_rms - gim_rms)
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 6), sharex=True, gridspec_kw={'height_ratios': [2, 1]})
    ax1.plot(doy_list, doris_rms, label='DORIS RMS', color='blue', linewidth=2)
    ax1.plot(doy_list, gim_rms, label='GIM RMS', color='orange', linestyle='--', linewidth=2)
    ax1.set_ylabel('RMS Value')
    ax1.legend(loc='upper right')
    ax1.grid(True)
    ax1.set_title('DORIS RMS and GIM RMS Over DOY')
    ax1.text(0.02, 0.95, f'Avg Difference: {mean_diff:.3f}', transform=ax1.transAxes, 
         fontsize=12, verticalalignment='top', color='red')
    # 下方子图：percent
    ax2.plot(doy_list, percent*100, label='Percent', color='green', linewidth=2)
    ax2.set_ylabel('Percentage (%)')
    ax2.set_xlabel('DOY (Day of Year)')
    ax2.set_ylim(percent_min - 5, percent_max + 5) 
    ax2.grid(True)
    ax2.set_title('Percentage Over DOY')
                    ...
                    for lat_range_idx, lat_range in enumerate(lat_range_list):
                        ...
                        for epoch_idx, epoch_time in enumerate(alt_epoch):
                            ...
                            if roti < roti_threshold:                             
                                if (indices := find_doris_indices(d_lon, d_lat, min_obs_count, 1, max_lat_gap, lat, lon)):
