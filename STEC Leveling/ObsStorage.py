...
class DORISStorage:
    ...
    def find_station(self, ID: int):
        for station in self.stations:
            if station.ID == ID:
                return station.station_code
            
        raise ValueError(f"{ID} not found in station list")
            ...
            with open(file, 'r') as rnxin:
                ...
                for line in rnxin:
                    ...
                    if not headerfinish:
                        ...
                        if   label.startswith("RINEX VERSION / TYPE"): pass
                        elif label.startswith("PGM / RUN BY / DATE"): pass

                        elif label.startswith("SATELLITE NAME"):
                            if temp[0:60].strip() == "JASON-3": prn = "L39"

                        elif label.startswith("OBSERVER / AGENCY"): pass
                            ...
                            ID = int(temp[1:3])
                            # only when the ID in STATION REFERENCE goes up one by one from zero (true for 3.00)
                            ...
                            if start_time.day == obs_epoch.day:
                                ...
                                for _ in range(obs_count):
                                    ...
                                    for line in range(line_per_obs):
                                        ...
                                        if line == 0:
                                            ID = int(temp[1:3])
                                            station_code = self.find_station(ID)
                                    ...
                                    DORISObs.PRN = prn
                                    obs = DORISObs(station_code, ID, obs_value, obs_epoch) 
